...
<head>
    ...
    <script>
        ...
        //添加
        function add() {
            var url = "<%=path%>/admin/spnews/news_Add.jsp";
            x_admin_show("添加", url, 1000, 660);
        }
        //查看
        function view(id) {
            var url = "<%=path%>/SPNewsServlet?action=view&id=" + id;
            x_admin_show("查看", url, 1000, 660);
        }
        //删除
        function del(obj, id) {
            layer.confirm('确认要删除吗？', function (index) {
                location.href = "<%=path%>/SPNewsServlet?action=delete&ids=" + id;
            });
        }
        //批量删除
        function plsc() {
            layer.confirm('确认要删除吗？', function (index) {
                ...
                if (ids == null) {
                    ...
                    location.href = "<%=path%>/SPNewsServlet?action=delete&ids=" + ids;
                }
        ...
        //编辑
        function edit(id) {
            var url = "<%=path%>/SPNewsServlet?action=editinit&id=" + id;
        ...
        function skip(topage) {
            location.href = "<%=path%>/SPNewsServlet?action=page&toPage=" + topage;
        }
        //跳转js
        function skip1() {
            if (checkRate($('#jumpnum').val())) {
                location.href = "<%=path%>/SPNewsServlet?action=page&toPage=" + $('#jumpnum').val();
            }
        }
        //判断输入的页码是否为数字
...
<body>
<div class="x-nav"><span class="layui-breadcrumb"> <a><cite>首页</cite></a> <a><cite>视频新闻</cite></a> </span></div>
<div class="x-body">
    <div class="tools">
        <ul class="toolbar">
    ...
    <table class="tablelist">
        ...
        <c:forEach var="list" items="${page.list}" varStatus="status">
            <tr>
                ...
                <td><a href="<%=path%>/upload/${list.SHIPIN}" target="_blank"/>${list.SHIPIN}</td>
